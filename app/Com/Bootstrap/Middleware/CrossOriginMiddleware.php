...
class CrossOriginMiddleware implements RequestMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if ($request->server('REQUEST_METHOD') == 'OPTIONS') {
            ...
            if (in_array($request->server('HTTP_HOST'), config('cross_origin.allow_domain_list'))) {
                $response->headers->set('Access-Control-Allow-Origin', config('cross_origin.doamin_schema') . $request->server('HTTP_HOST'));
